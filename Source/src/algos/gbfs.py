...
class GBFS(Search):
    def __init__(
        ...
        # Chỉ dùng heuristic value, không cần gval
    ...
    def expand(
        ...
        for dir in Direction:
            if self.can_go(state, dir):
                ...
                # Cập nhật fval bằng pure heuristic
                new_state.fval = self.heuristic(new_state.stones_pos, self.switches_pos)
    ...
    def hungarian_heuristic(
        ...
        for i, (sx, sy, _) in enumerate(stones_pos):
            for j, (gx, gy) in enumerate(switches_pos):
                dists[i, j] = manhattan(sx, sy, gx, gy)
